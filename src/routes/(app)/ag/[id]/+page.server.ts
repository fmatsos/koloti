...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const { data: ag } = await supabase
		.from('assembly')
		.select('id, title, type, mode, status, scheduled_at, location, quorum_pct, agenda_item(position, title, description, requires_vote)')
		.eq('id', params.id)
		.single();

	if (!ag) throw error(404, 'Assemblée introuvable');

	const agendaItems = (Array.isArray(ag.agenda_item) ? ag.agenda_item : [])
		.sort((a: { position: number }, b: { position: number }) => a.position - b.position);

	let quorum = null;
	if (ag.status === 'open') {
		const { data: properties } = await supabase.from('property').select('id, vote_weight');
		const { data: attendances } = await supabase.from('attendance').select('property_id, mode').eq('assembly_id', params.id);
