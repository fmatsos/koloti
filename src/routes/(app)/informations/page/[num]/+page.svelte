		...
		<article class="post-card">
			<div class="post-header">
				<div class="title-row">
					...
					{#if data.isAdminOrEditor && !post.is_published}<span class="badge-draft">Brouillon</span>{/if}
					{#if data.isAdminOrEditor}<a href="/informations/{post.id}/edit" class="edit-link">Modifier</a>{/if}
				</div>
				<div class="meta">
					{#if post.published_at}
						<time datetime={post.published_at}>{new Date(post.published_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })}</time>
					{/if}
...
<nav class="pagination">
	<a href={data.currentPage === 2 ? '/informations' : `/informations/page/${data.currentPage - 1}`} class="page-link">← Précédent</a>
...
<style>
	...
	.edit-link, .read-more, .page-link { color: var(--color-primary, #1a73e8); }
	.edit-link { font-size: 0.75rem; margin-left: auto; }
	.read-more { display: inline-block; margin-top: 0.75rem; font-size: 0.875rem; }
	...
	.meta, .page-info { font-size: 0.8125rem; color: var(--color-text-muted, #6b7280); }
