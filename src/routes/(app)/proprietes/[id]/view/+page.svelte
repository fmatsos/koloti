...
<div class="detail-grid">
	<div class="card">
		<h2>Informations</h2>
		<dl class="info-list">
			<dt>Référence</dt><dd>{prop.reference}</dd>
			<dt>N° de voie</dt><dd>{prop.street_number ?? '—'}</dd>
			<dt>Voie</dt><dd>{prop.street_name ?? '—'}</dd>
			<dt>N° cadastral</dt><dd>{prop.cadastre_number ?? '—'}</dd>
		</dl>
	</div>
				...
				<ul class="owner-list">
					{#each current as ownership (ownership.id)}
						...
						<li>
							...
							<br /><small class="muted">{profile?.email ?? ''} · depuis {ownership.start_date}</small>
						</li>
					{/each}
				</ul>
			{/if}
...
<style>
	...
	.btn-sm { padding: 0.375rem 0.875rem; border-radius: 0.25rem; font-size: 0.8125rem; text-decoration: none; }
