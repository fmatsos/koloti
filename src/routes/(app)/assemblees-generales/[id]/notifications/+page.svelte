...
<div class="mb-6">
    <a href="/assemblees-generales/{ag.id}" class="inline-block mb-2 text-xs no-underline text-surface-500">← {ag.title}</a>
...
<div class="flex items-center gap-3 flex-wrap mb-4">
    <span class="badge preset-tonal-success text-xs rounded-full px-2 py-0.5">{summary.sent} envoyé{summary.sent !== 1 ? 's' : ''}</span>
    <span class="badge preset-tonal-error text-xs rounded-full px-2 py-0.5">{summary.failed} échec{summary.failed !== 1 ? 's' : ''}</span>
    <span class="badge preset-tonal-warning text-xs rounded-full px-2 py-0.5">{summary.pending} en attente</span>
...
{#if notifications.length === 0}
    <div class="card preset-filled-surface-50-950 rounded-xl border border-surface-200-800 shadow-sm mt" role="status">
        <p class="text-surface-500">Aucune notification enregistrée pour cette AG.</p>
    </div>
{:else}
    <div class="card preset-filled-surface-50-950 rounded-xl border border-surface-200-800 shadow-sm mt">
