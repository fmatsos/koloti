			...
			<table class="table w-full text-sm">
				<thead>
					<tr class="border-b border-surface-200-800">
						<th class="text-left text-xs font-semibold uppercase tracking-wider text-surface-500 px-5 py-3">Login</th>
						<th class="text-left text-xs font-semibold uppercase tracking-wider text-surface-500 px-4 py-3">Nom</th>
						<th class="text-left text-xs font-semibold uppercase tracking-wider text-surface-500 px-4 py-3">Email</th>
						<th class="text-left text-xs font-semibold uppercase tracking-wider text-surface-500 px-4 py-3">Rôle</th>
						<th class="text-left text-xs font-semibold uppercase tracking-wider text-surface-500 px-4 py-3">Statut</th>
						<th class="text-left text-xs font-semibold uppercase tracking-wider text-surface-500 px-4 py-3">Dernière connexion</th>
					...
					{#each data.comptes as compte (compte.id)}
						...
						<tr class="hover:bg-surface-100-900 transition-colors">
							<td class="px-5 py-3.5">
								<code class="bg-surface-200-800 px-1.5 py-0.5 rounded text-xs">{cred?.login ?? '—'}</code>
							</td>
							...
							<td class="px-4 py-3.5 text-surface-600-400">{roleLabels[compte.role as keyof typeof roleLabels]}</td>
							<td class="px-4 py-3.5">
								<span class="badge {statusPresets[compte.status as StatusKey] ?? 'preset-tonal-surface'} text-xs rounded-full px-2.5 py-0.5">
									{statusLabels[compte.status as StatusKey]}
								</span>
							</td>
							<td class="px-4 py-3.5 text-surface-500 text-xs">{fmt(compte.last_login_at)}</td>
							<td class="px-4 py-3.5">
								<a href="/comptes/{compte.id}/view" class="text-primary-500 hover:underline text-xs font-medium">Voir →</a>
							</td>
						</tr>
					{/each}
				</tbody>
