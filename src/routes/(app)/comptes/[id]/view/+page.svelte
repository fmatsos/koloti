...
{#if data.created}<div class="alert alert-success">Compte créé. Le lien d'activation a été envoyé.</div>{/if}
{#if form?.attachSuccess}<div class="alert alert-success">Propriété rattachée avec succès.</div>{/if}
...
<div class="detail-grid">
	<div class="card">
		<h2>Informations</h2>
		<dl class="info-list">
			<dt>Login</dt><dd><code>{cred?.login ?? '—'}</code></dd>
			<dt>Email</dt><dd>{compte.email}</dd>
			<dt>Téléphone</dt><dd>{compte.phone ?? '—'}</dd>
			<dt>Rôle</dt><dd>{roleLabels[compte.role]}</dd>
			<dt>Statut</dt><dd><span class="badge badge-{compte.status}">{statusLabels[compte.status]}</span></dd>
			<dt>Créé le</dt><dd>{fmt(compte.created_at)}</dd>
			<dt>Activé le</dt><dd>{fmt(compte.activated_at)}</dd>
			<dt>Dernière connexion</dt><dd>{fmt(compte.last_login_at)}</dd>
		</dl>
	</div>
	...
	<div class="card">
		...
		{:else}
			<ul class="prop-list">
				{#each activeOwn as ownership (ownership.id)}
					{@const prop = Array.isArray(ownership.property) ? ownership.property[0] : ownership.property}
					<li>
		...
		<form method="POST" action="?/findProperty">
			<div class="field">
				...
				<input id="property_reference" name="property_reference" type="text" required placeholder="ex. LOT-12" />
			</div>
...
<dialog bind:this={dialog}>
	...
	<form method="POST" action="?/confirmTransfer">
		...
		<div class="dialog-actions">
			<button type="button" onclick={() => dialog.close()} class="btn-outline btn-sm">Annuler</button>
...
<style>
	...
	h2, h3 { margin: 0 0 1rem; font-size: 1rem; }
	...
	.info-list { display: grid; grid-template-columns: auto 1fr; gap: 0.5rem 1rem; margin: 0; font-size: 0.875rem; }
	dt { color: var(--color-text-muted, #6b7280); font-weight: 500; }
	dd { margin: 0; }
	...
	.prop-list { list-style: none; margin: 0 0 1rem; padding: 0; font-size: 0.875rem; }
	.prop-list li { padding: 0.3rem 0; border-bottom: 1px solid #f3f4f6; }
	.muted, .empty { color: var(--color-text-muted, #6b7280); }
	hr { border: none; border-top: 1px solid var(--color-border, #e5e7eb); margin: 1rem 0; }
	.field { margin-bottom: 0.75rem; }
	label { display: block; font-size: 0.8125rem; font-weight: 500; margin-bottom: 0.25rem; }
	input[type='text'], input[type='date'] { width: 100%; padding: 0.4rem 0.625rem; border: 1px solid var(--color-border, #e5e7eb); border-radius: 0.25rem; font-size: 0.875rem; box-sizing: border-box; }
	.confirm-box { margin-top: 1rem; padding: 0.75rem; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 0.25rem; }
	dialog { border-radius: 0.5rem; border: 1px solid var(--color-border, #e5e7eb); padding: 1.5rem; max-width: 480px; width: 90%; }
	dialog::backdrop { background: rgba(0, 0, 0, 0.4); }
	.dialog-actions { display: flex; gap: 0.5rem; justify-content: flex-end; margin-top: 1rem; }
	.btn-sm { padding: 0.375rem 0.875rem; background: var(--color-primary, #1a73e8); color: white; border: none; border-radius: 0.25rem; font-size: 0.8125rem; cursor: pointer; text-decoration: none; }
	.btn-outline { background: white; color: #374151; border: 1px solid #d1d5db; }
	.alert { padding: 0.75rem 1rem; border-radius: var(--radius, 0.375rem); margin-bottom: 1rem; font-size: 0.875rem; }
