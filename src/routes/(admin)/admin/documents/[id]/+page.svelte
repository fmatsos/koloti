...
<div class="detail-grid">
	<div class="card">
		<h2>Informations</h2>
		<dl>
			<dt>Type</dt><dd>{typeLabels[doc.type] ?? doc.type}</dd>
			<dt>Année</dt><dd>{doc.year ?? '—'}</dd>
			<dt>Taille</dt><dd>{formatSize(doc.size_bytes)}</dd>
			<dt>MIME</dt><dd class="muted">{doc.mime_type ?? '—'}</dd>
			<dt>Ajouté le</dt><dd>{new Date(doc.created_at).toLocaleDateString('fr-FR')}</dd>
			{#if doc.description}<dt>Description</dt><dd>{doc.description}</dd>{/if}
		</dl>
	...
	<div class="card">
		...
		<div class="danger-zone">
			<form method="POST" action="?/delete" onsubmit={(e) => { if (!confirm('Supprimer définitivement ce document ?')) e.preventDefault(); }}>
...
<style>
	...
	h1 { margin: 0 0 0.25rem; font-size: 1.5rem; word-break: break-word; }
	...
	dl { display: grid; grid-template-columns: auto 1fr; gap: 0.25rem 1rem; font-size: 0.875rem; margin: 0 0 1rem; }
	dt { font-weight: 500; color: var(--color-text-muted, #6b7280); }
	dd { margin: 0; }
	.muted { color: var(--color-text-muted, #6b7280); }
	.field { margin-bottom: 0.75rem; }
	select { width: 100%; padding: 0.4rem 0.625rem; border: 1px solid var(--color-border, #e5e7eb); border-radius: 0.25rem; font-size: 0.875rem; }
	...
	.btn-download { display: inline-block; margin-top: 0.5rem; padding: 0.375rem 0.875rem; background: #16a34a; color: white; border-radius: 0.25rem; font-size: 0.8125rem; text-decoration: none; }
	.danger-zone { margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid #fee2e2; }
