...
<div class="page-header">
	<a href="/admin" class="back-link">← Accueil admin</a>
	<div class="header-row">
		<h1>État nominatif</h1>
		<a href="/admin/etat-nominatif/export.csv" class="btn-export" download>Exporter CSV</a>
	</div>
</div>
...
<div class="card">
	...
	{:else}
			...
			<tbody>
				{#each data.properties as prop}
					{#if prop.activeOwnerships.length === 0}
						<tr>
							<td><a href="/admin/proprietes/{prop.id}">{prop.reference}</a></td>
							<td>{prop.address ?? '—'}</td>
							<td class="center">{prop.vote_weight}</td>
							<td class="empty-cell">—</td>
						</tr>
					{:else}
						{#each prop.activeOwnerships as o, i}
							...
							<tr class={i > 0 ? 'continuation' : ''}>
								{#if i === 0}
									<td rowspan={prop.activeOwnerships.length}>
										<a href="/admin/proprietes/{prop.id}">{prop.reference}</a>
									</td>
									<td rowspan={prop.activeOwnerships.length}>{prop.address ?? '—'}</td>
		...
		<p class="count">{data.properties.length} propriété(s) · {data.properties.reduce((s, p) => s + p.activeOwnerships.length, 0)} propriétaire(s) actif(s)</p>
	{/if}
</div>
...
<style>
	...
	.btn-export { padding: 0.375rem 0.875rem; background: #16a34a; color: white; border-radius: 0.25rem; font-size: 0.8125rem; text-decoration: none; white-space: nowrap; }
	...
	.continuation td { border-top: none; }
	.center { text-align: center; }
	.empty-cell { color: var(--color-text-muted, #6b7280); }
	...
	.count { margin: 0.75rem 0 0; font-size: 0.8125rem; color: var(--color-text-muted, #6b7280); text-align: right; }
	a { color: var(--color-primary, #1a73e8); }
</style>
