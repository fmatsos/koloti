...
<div class="page-header">
	<a href="/admin/assemblees-generales/{data.ag.id}" class="back-link">← {data.ag.title}</a>
	<h1>Émargement</h1>
</div>
...
<div class="quorum-bar {qr.reached ? 'reached' : 'pending'}">
	<span
		>Quorum : {qr.presentWeight} / {qr.totalWeight} voix ({qr.ratio.toFixed(1)} %) — requis {qr.quorumPct}
		%</span
	>
	{#if qr.reached}
						...
						{#if prop.ownerId}
							<form method="POST" class="inline-form">
								...
								>
									...
									<option value="present" selected={prop.attendance?.mode === 'present'}
										>Présent</option
									>
									<option value="represented" selected={prop.attendance?.mode === 'represented'}
										>Représenté</option
									>
									<option value="absent" selected={prop.attendance?.mode === 'absent'}
										>Absent</option
									>
								</select>
							</form>
