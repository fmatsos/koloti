...
{#if form?.success}
	<div class="alert-success">{form.sent} lien(s) d'activation envoyé(s).{#if form.errors?.length} {form.errors.length} erreur(s).{/if}</div>
{/if}
...
<div class="form-card">
	...
	{:else}
		<p class="info">Les {data.pending.length} comptes suivants n'ont pas encore activé leur compte. Un nouveau lien d'activation leur sera envoyé (le précédent sera invalidé).</p>
		...
		<div class="warning">
			<strong>Confirmation requise :</strong> Cette action enverra un email à chacun de ces {data.pending.length} compte(s). Les liens précédents seront révoqués.
		</div>
...
<style>
	...
	.info { font-size: 0.9375rem; margin: 0 0 1rem; }
	.account-list { list-style: none; padding: 0; margin: 0 0 1.25rem; font-size: 0.875rem; }
	.account-list li { padding: 0.375rem 0; border-bottom: 1px solid #f3f4f6; }
