...
export const actions: Actions = {
	add: async ({ request, locals, params }) => {
		...
		if (ag?.status !== 'draft') return fail(400, { error: "L'ordre du jour ne peut être modifié que si l'AG est en brouillon." });
		...
		const { count } = await supabase.from('agenda_item').select('id', { count: 'exact', head: true }).eq('assembly_id', params.id);
		...
		await writeAuditLog({ actorId: locals.profile.id, action: 'agenda_item.add', entity: 'agenda_item', payload: { assembly_id: params.id, title: parsed.data.title } });
		return { success: true };
	},
	...
	update: async ({ request, locals, params }) => {
		...
		const supabase = createServiceClient();
		const { data: ag } = await supabase.from('assembly').select('status').eq('id', params.id).single();
		if (ag?.status !== 'draft') return fail(400, { error: "Modification impossible hors brouillon." });

		const parsed = updateSchema.safeParse(Object.fromEntries(await request.formData()));
		if (!parsed.success) return fail(400, { error: parsed.error.issues[0]?.message ?? 'Invalide.' });

		const { error: err } = await supabase.from('agenda_item').update({
			title: parsed.data.title,
			description: parsed.data.description || null,
			requires_vote: parsed.data.requires_vote
		}).eq('id', parsed.data.item_id).eq('assembly_id', params.id);
	...
	delete: async ({ request, locals, params }) => {
		...
		const supabase = createServiceClient();
		const { data: ag } = await supabase.from('assembly').select('status').eq('id', params.id).single();
		if (ag?.status !== 'draft') return fail(400, { error: "Suppression impossible hors brouillon." });
		...
		const { data: deleted } = await supabase.from('agenda_item').delete().eq('id', parsed.data.item_id).eq('assembly_id', params.id).select('position').single();
		if (deleted) {
			const { data: remaining } = await supabase.from('agenda_item').select('id, position').eq('assembly_id', params.id).order('position');
			for (let i = 0; i < (remaining ?? []).length; i++) {
				await supabase.from('agenda_item').update({ position: i + 1 }).eq('id', remaining![i].id);
			}
		}
		return { success: true };
	},
	...
	move: async ({ request, locals, params }) => {
		...
		const { data: item } = await supabase.from('agenda_item').select('id, position').eq('id', parsed.data.item_id).single();
		if (!item) return fail(404, { error: 'Point introuvable.' });

		const swapPos = parsed.data.direction === 'up' ? item.position - 1 : item.position + 1;
		const { data: neighbor } = await supabase.from('agenda_item').select('id').eq('assembly_id', params.id).eq('position', swapPos).single();
