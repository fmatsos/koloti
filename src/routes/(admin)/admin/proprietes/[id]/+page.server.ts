...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const { data: prop } = await supabase
		.from('property')
		.select('*, ownership(id, profile_id, start_date, end_date, is_primary, profile:profile_id(full_name, email, status))')
		.eq('id', params.id)
		.single();
...
export const actions: Actions = {
	update: async ({ request, locals, params }) => {
		...
		const supabase = createServiceClient();
		await supabase.from('property').update({ ...parsed.data, address: parsed.data.address || null }).eq('id', params.id);
		await writeAuditLog({ actorId: locals.profile.id, action: 'property.update', entity: 'property', entityId: params.id, payload: parsed.data });
		return { success: true };
	}
};
